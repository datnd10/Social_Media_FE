...
export const createNontification = (nontificationData, postId, userId) => async (dispatch) => {
    dispatch({type: CREATE_NONTIFICATION_REQUEST});
    try {
        const {data} = await api.post(`/api/nontification/post/${postId}/from/${userId}`, nontificationData);
...
export const getUserNontification = () => async (dispatch) => {
    dispatch({type: GET_NONTIFICATION_USER_REQUEST});
    try {
        const {data} = await api.get(`/api/nontification`);
